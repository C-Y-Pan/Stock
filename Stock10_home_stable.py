...
def run_simple_strategy(data, buy_threshold=60, sell_threshold=30, fee_rate=0.001425, tax_rate=0.003):
    """
    策略執行核心 v10.1 (Fix Buy/Sell Markers):
    - [修復] 增加 Buy_Y 與 Sell_Y 計算，解決圖表標記消失問題。
    - [調整] 預設 sell_threshold 提高至 30，避免死抱。
    """
    df = data.copy()
    ...
    lows = df['Low'].values # 讀取最低價算座標
    highs = df['High'].values # 讀取最高價算座標
    ...
    for i in range(len(df)):
        ...
        if position == 1:
            ...
            # 1. 分數低於賣出門檻 (轉弱)
            # 2. 硬性停損 (虧損 > 15%)
            is_stop_loss = (pnl_pct < -0.15)
            is_score_sell = (score < sell_threshold)
            
            if is_stop_loss:
                signal = 0; action = "Sell"; reason = f"硬性停損 ({pnl_pct*100:.1f}%)"
            elif is_score_sell:
                signal = 0; action = "Sell"; reason = f"評分轉弱 ({int(score)}分)"
            else:
                signal = 1; action = "Hold"; reason = f"續抱 ({int(score)}分)"
                
            # 結算
            if signal == 0:
                final_pnl = (price + cum_div - entry_price) / entry_price * 100
                sign = "+" if final_pnl > 0 else ""
                ret_label = f"{sign}{final_pnl:.1f}%"
        ...
        # --- 空手狀態 ---
            ...
            if score >= buy_threshold:
                ...
                if score >= 80: reason = "🔥 極強勢買進"
                elif score >= 60: reason = "🚀 趨勢確立"
                else: reason = "✅ 試單買進"
            else:
                signal = 0
                action = "Wait"
...
def run_optimization(raw_df, market_df, user_start_date, fee_rate=0.001425, tax_rate=0.003, use_chip_strategy=True, use_strict_bear_exit=True):
    ...
    for thresh in thresholds:
        df_res = run_simple_strategy(
            ...
            sell_threshold=fixed_sell_threshold,  # <--- 設定為 20
            fee_rate=fee_rate, 
            tax_rate=tax_rate
        )
...
def calculate_alpha_score(df, margin_df, short_df):
    """
    Alpha Score v8.0 (Analog Strategy Core):
    將所有買賣規則 (Strategy A/B/C/D + Filters) 轉化為數值評分。
    
    評分邏輯：
    - 趨勢架構 (Trend): +/- 40分
    - 動能籌碼 (Momentum): +/- 30分
    - 策略特徵 (Patterns): + 30分 (原有策略轉化)
    - 風險懲罰 (Risk): - 50分 (原有濾網轉化)
    
    決策建議：
    - Score >= 60: Strong Buy (買進/加碼)
    - Score >= 0: Hold (續抱)
    - Score < 0: Sell (賣出/空手)
    """
    df = df.copy()
    ...
    if 'OBV' not in df.columns: df['OBV'] = (np.sign(df['Close'].diff()) * df['Volume']).fillna(0).cumsum()
    if 'OBV_MA20' not in df.columns: df['OBV_MA20'] = df['OBV'].rolling(20).mean()
    if 'BB_Lower' not in df.columns: 
        std = df['Close'].rolling(20).std()
        df['BB_Lower'] = df['MA20'] - 2 * std
    if 'SuperTrend' not in df.columns: df['SuperTrend'] = df['MA20'] # 防呆 fallback

    # 轉為 Numpy Arrays 加速運算
    ...
    # 趨勢指標 (Trend Trend is King)
    # 使用 SuperTrend 作為核心趨勢判定 (若 Close > SuperTrend 代表多方)
    is_supertrend_bull = (close > supertrend)
    ...
    # ==========================================
    # 2. 趨勢與架構評分 (Trend Score) - Max: +/- 40
    # ==========================================
    score_trend = np.zeros(len(df))
    
    # A. 均線位置 (各 +5 / -5)
    score_trend += np.where(close > ma20, 5, -5)   # 月線守門員
    score_trend += np.where(close > ma60, 10, -10) # 季線生命線 (權重較高)
    score_trend += np.where(close > ma240, 5, -5)  # 年線多空分界
    
    # B. SuperTrend (趨勢指標)
    score_trend += np.where(is_supertrend_bull, 15, -15) # 核心趨勢
    ...
    # ==========================================
    # 3. 動能與籌碼評分 (Momentum Score) - Max: +/- 30
    # ==========================================
    score_mom = np.zeros(len(df))
    
    # A. RSI 動能 (線性對應)
    # RSI 50=0分, 70=+10分, 30=-10分
    score_mom += np.clip((rsi - 50) * 0.5, -10, 10)
    
    # B. 成交量能 (Volume)
    # 價漲量增 (+5), 價跌量縮 (+5 -> 健康), 價跌量增 (-5 -> 出貨)
    price_change = pd.Series(close).diff().fillna(0).values
    vol_ratio = vol / vol_ma
    
    cond_vol_bull = (price_change > 0) & (vol_ratio > 1.0)
    cond_vol_pullback = (price_change < 0) & (vol_ratio < 0.8)
    cond_vol_bear = (price_change < 0) & (vol_ratio > 1.2)
    
    score_mom += np.where(cond_vol_bull, 5, 0)
    score_mom += np.where(cond_vol_pullback, 3, 0) # 健康回檔加分
    score_mom += np.where(cond_vol_bear, -5, 0)
    
    # C. OBV 籌碼 (主力動向)
    score_mom += np.where(obv > obv_ma, 10, -5)
    
    # ==========================================
    # 4. 策略型態加分 (Pattern Bonuses) - Max: +35
    # ==========================================
    ...
    # --- 策略 B: 均線回測 (Pullback) ---
    # 條件: 趨勢多 + 守月線 + 量縮 + 觸及月線附近
    # (low <= ma20*1.02) 代表回測到支撐區
    ...
    # --- 策略 C: 籌碼佈局 (Accumulation) ---
    # 條件: 站穩季線 + OBV強 + 量縮整理 (可能跌破月線或RSI弱，但在季線上)
    cond_strat_c = (close > ma60) & (obv > obv_ma) & (vol < vol_ma) & ((close < ma20) | (rsi < 55)) & (close > bb_lower)
    score_pattern += np.where(cond_strat_c, 10, 0)
    
    # --- 策略 D: 超賣反彈 (Reversal) ---
    # 這是一個逆勢策略，需要給予大量的分數來抵銷前面的「趨勢扣分」(因為跌破均線會被扣很多分)
    # 條件: RSI低 + 破布林下緣 + 有量
    cond_strat_d = (rsi < 30) & (close < bb_lower) & (vol > vol_ma * 0.5)
    # 給予 +45 分，強力扭轉空頭評分至正值 (嘗試抄底)
    ...
    cond_breakout = (close >= high_100d) & (weekly_ratio < 1.27)
    score_pattern += np.where(cond_breakout, 10, 0)

    # ==========================================
    # 5. 風險懲罰與濾網 (Penalties) - Max: -50
    # ==========================================
    score_penalty = np.zeros(len(df))
    penalty_log = [[] for _ in range(len(df))]
    
    # A. 糾結雜訊 (Noise)
    # 計算均線糾結度
    ma_stack = np.vstack([ma60, ma120, ma240])
    ma_max_val = np.max(ma_stack, axis=0)
    ma_min_val = np.min(ma_stack, axis=0)
    congestion_idx = np.divide((ma_max_val - ma_min_val), close, out=np.ones_like(close), where=close!=0)
    
    # 若糾結度 < 3%，扣分 (除非是剛發散)
    is_squeeze = congestion_idx < 0.03
    score_penalty += np.where(is_squeeze, -10, 0)
    
    # B. 崩盤濾網 (Crash)
    # 年線下彎斜率過大，強力扣分，禁止做多
    is_crash = ma240_slope <= -0.001
    score_penalty += np.where(is_crash, -50, 0) # 直接判死刑
    
    # C. 三線空頭 (Triple Bear)
    # 季/半/年線全下彎
    is_triple_bear = (ma60_slope < 0) & (ma120_slope < 0) & (ma240_slope < 0)
    # 例外：若呈現發散噴出狀態 (價格 > 所有均線 且 糾結度擴大) 則不扣分
    congestion_slope = pd.Series(congestion_idx).diff().fillna(0).values
    is_valid_reversal = (congestion_slope > 0) & (close > ma60) & (close > ma120) & (close > ma240)
    
    score_penalty += np.where(is_triple_bear & (~is_valid_reversal), -30, 0)
    
    # D. 高檔爆量滯漲 (Distribution)
    # 量 > 2.5倍均量 但 漲幅 < 0.5%
    cond_distribution = (vol > vol_ma * 2.5) & (pd.Series(close).pct_change().fillna(0) < 0.005)
    ...
    for i in range(len(df)):
        s = final_score_smoothed[i]
        
        # 組裝 HTML
        txt = f"<b>總分: <span style='color:{C_POS if s>0 else C_NEG}'>{int(s)}</span></b><br>"
        txt += f"<span style='color:#666'>──────────</span><br>"
        txt += f"趨勢: {int(score_trend[i]):+d} | 動能: {int(score_mom[i]):+d}<br>"
        
        bonuses = score_pattern[i]
        if bonuses > 0:
            txt += f"<b><span style='color:{C_POS}'>加分: +{int(bonuses)}</span></b><br>"
            if cond_strat_a[i]: txt += " • 動能突破 (+15)<br>"
            if cond_strat_b[i]: txt += " • 均線回測 (+15)<br>"
            if cond_strat_c[i]: txt += " • 籌碼佈局 (+10)<br>"
            if cond_strat_d[i]: txt += " • 超賣反彈 (+45)<br>"
            if cond_breakout[i]: txt += " • 百日突破 (+10)<br>"
            
        penalties = score_penalty[i]
        if penalties < 0:
            txt += f"<b><span style='color:{C_NEG}'>扣分: {int(penalties)}</span></b><br>"
            if is_squeeze[i]: txt += " • 均線糾結 (-10)<br>"
            if is_crash[i]: txt += " • 崩盤趨勢 (-50)<br>"
            if is_triple_bear[i] and not is_valid_reversal[i]: txt += " • 三線空頭 (-30)<br>"
        ...
        # 簡單日誌
        log_txt = "盤整"
        if s >= 60: log_txt = "🔥 強力買進"
        elif s >= 20: log_txt = "📈 偏多操作"
        elif s <= -60: log_txt = "⚡ 強力賣出"
        elif s <= -20: log_txt = "📉 偏空調節"
        
        # 覆蓋特殊策略標籤
        if cond_strat_d[i]: log_txt = "💎 抄底機會"
        if is_crash[i]: log_txt = "☠️ 崩盤迴避"
        
        log_msg.append(log_txt)
    ...
    if ticker_input: 
        with st.spinner(f'正在分析 {ticker_input} ...'):
            ...
            if final_df is None or final_df.empty:
                ...
                with tab1:
                    # ==========================================
                    # 1. 資料前處理 (Data Pre-processing)
                    # ==========================================
                    # 確保 Alpha Score 相關欄位存在
                    final_df['Alpha_Score'] = stock_alpha_df['Alpha_Score']
                    final_df['Score_Detail'] = stock_alpha_df.get('Score_Detail', "") # 防呆
                    ...
                    # ==========================================
                    # 2. 建立多子圖框架 (Create Subplots)
                    # ==========================================
                    fig = make_subplots(
                        rows=6, cols=1, 
                        shared_xaxes=True, 
                        vertical_spacing=0.02, 
                        row_heights=[0.35, 0.15, 0.10, 0.15, 0.15, 0.10], 
                        subplot_titles=(
                            "", 
                            "買賣評等 (Alpha Score)", 
                            "評分動能 (Alpha Slope)", 
                    ...
                    # B. 均線
                    fig.add_trace(go.Scatter(x=final_df['Date'], y=final_df['MA20'], mode='lines', line=dict(color='yellow', width=1), name='月線'), row=1, col=1)
                    fig.add_trace(go.Scatter(x=final_df['Date'], y=final_df['MA60'], mode='lines', line=dict(color='orange', width=1.5), name='季線'), row=1, col=1)
                    ...
                    # C. 買進訊號 (Buy Markers)
                    # [關鍵修正] 使用 Buy_Y 確保標記位置正確
                    buy_signals = final_df[final_df['Action'] == 'Buy']
                    if not buy_signals.empty:
                        fig.add_trace(go.Scatter(
                            ...
                            name='買進訊號',
                            hovertext=buy_signals['Reason']
                        ), row=1, col=1)

                    # D. 賣出訊號 (Sell Markers)
                    # [關鍵修正] 使用 Sell_Y 並加上 Return_Label 文字
                    sell_signals = final_df[final_df['Action'] == 'Sell']
                    if not sell_signals.empty:
                        fig.add_trace(go.Scatter(
                            x=sell_signals['Date'], 
                            y=sell_signals['Sell_Y'], 
                            mode='markers+text', 
                            text=sell_signals['Return_Label'], # 顯示報酬率 (例如 +10.5%)
                            textposition="top center",
                            textfont=dict(color='white', size=11, family="Arial Black"),
                            marker=dict(symbol='triangle-down', size=14, color='#FF00FF', line=dict(width=1, color='black')),
                            name='賣出訊號',
                            hovertext=sell_signals['Reason']
                        ), row=1, col=1)

                    # ==========================================
                    # 4. Row 2: Alpha Score
                    # ==========================================
                    colors_score = ['#ef5350' if v > 0 else ('#26a69a' if v < 0 else 'gray') for v in final_df['Alpha_Score']]
                    fig.add_trace(go.Bar(
                        x=final_df['Date'], 
                        y=final_df['Alpha_Score'], 
                        name='Alpha Score', 
                        marker_color=colors_score,
                        hovertext=final_df['Score_Detail'], # 綁定 HTML 詳細理由
                        hoverinfo="x+text" 
                    ), row=2, col=1)
                    
                    # 畫出 60分 (買進線) 與 0分 (多空線)
                    fig.add_hline(y=60, line_dash="dot", line_color="red", row=2, col=1, annotation_text="強力買進區")
                    fig.add_hline(y=0, line_width=1, line_color="white", row=2, col=1)
                    fig.update_yaxes(range=[-110, 110], row=2, col=1)

                    # ==========================================
                    # 5. Row 3: Alpha Slope (動能變化)
                    # ==========================================
                    colors_slope = ['#ef5350' if v > 0 else '#26a69a' for v in final_df['Alpha_Slope']]
                    fig.add_trace(go.Bar(x=final_df['Date'], y=final_df['Alpha_Slope'], name='評分動能', marker_color=colors_slope), row=3, col=1)
                    ...
                    # OBV 疊加 (使用雙軸)
                    fig.add_trace(go.Scatter(x=final_df['Date'], y=final_df['OBV'], mode='lines', line=dict(color='yellow', width=1), name='OBV', yaxis="y2"), row=4, col=1)

                    # ==========================================
                    # 7. Row 5: RSI
                    # ==========================================
                    fig.add_trace(go.Scatter(x=final_df['Date'], y=final_df['RSI'], name='RSI', line=dict(color='cyan', width=1.5)), row=5, col=1)
                    fig.add_hline(y=50, line_width=1, line_color="gray", row=5, col=1)
                    fig.add_hrect(y0=70, y1=100, fillcolor="red", opacity=0.1, layer="below", row=5, col=1)
                    fig.add_hrect(y0=0, y1=30, fillcolor="green", opacity=0.1, layer="below", row=5, col=1)

                    # ==========================================
                    # 8. Row 6: 均線糾結度
                    # ==========================================
                    colors_gap = ['#ef5350' if v < 5 else '#00e676' for v in final_df['Congestion_Index']]
                    fig.add_trace(go.Bar(x=final_df['Date'], y=final_df['Congestion_Index'], name='糾結度(%)', marker_color=colors_gap), row=6, col=1)
                    fig.add_hline(y=5, line_dash="dash", line_color="red", row=6, col=1)
                    ...
                    # 設定 OBV 的副座標軸 (Row 4)
                    fig.update_layout(yaxis4=dict(title="成交量", side="left"), yaxis7=dict(title="OBV", side="right", overlaying="y4", showgrid=False))
